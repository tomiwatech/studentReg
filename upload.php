...
if (isset($_POST['filename']) && isset($_POST['description'])) {
     ...
     // allowable file extensions
         $allow_ext = array('jpeg','jpg','png','gif','pdf');

         $user_pic = time().'_'.rand(1000,9999).'.'.$file_ext;

         if(in_array($file_ext, $allow_ext)) {
                   if($size <= $max ){
                    move_uploaded_file($file_tmp, $upload_dir.$user_pic);
                   }else if($size>$max){
                      $error_message = "Size too large";
                   }
         }else{
                $error_message = "Please select a valid image extension";
           }


           //check if no error message then insert into db
           if (!isset($error_message)) {
        $sql = "INSERT INTO uploads(filename,description,file,matric) VALUES('$filename','$description','$user_pic','$matric')";
                 if ($result = mysql_query($sql)) {
                   $successMsg = "Your file was uploaded successfully";
                 }else{
...
<marquee behavior="alternate"><h5 style="color:#0ac29d;"> UPLOAD ALL FILES HERE</h5></marquee>
                 ...
                 <form method="post" action="upload.php" enctype="multipart/form-data">
                        ...
                        <td>
                                        ...
                                        <div class="form-group">
                                                ...
                                                <div class="col-md-9">
                      <input type="text" name="description" class="form-control" placeholder="Enter file description" required>
                                              </div>
                                        </div>
                        </td>
                          ...
                          <td>
<button type="submit" style="background: #0ac29d;color: white;width: 100px;" name="button" class="btn green" >Add File</button>
                          </td>
                    </tr>
                  </form>
                  <?php 
                     $sql = "SELECT * FROM uploads WHERE matric='$matric'";
                     $result = @mysql_query($sql);
                  ...
                  ?>
                    ...
                    <tr>
                      ...
                      <td><img src="<?php echo $upload_dir.$row['file'];?>" height="80"></td>
<td><a type="button" href="upload.php?delete=<?php echo $row['id']?>" style="background:#ec4657;border-color:#ec4657;color: white;width: 100px;" class="btn" onclick="return confirm('Are you sure you want to delete this record?')">Delete</a></td>
                    </tr>
